pro I6_batch_avirisng2017_samclass


  envi, /restore_base_save_files
  envi_batch_init, log_file='batch.log'

  startfile = 10         ; begin processing at this file number
  ...
  outdirectory = 'X:\delta_sav\raster\classification\SAM_SMA\201711\SAM\'
  outclassdir  = 'X:\delta_sav\raster\classification\SAM_SMA\201711\SAM\class_files\'
  ; spectral library path and filename
  slifilename  = 'X:\delta_sav\raster\classification\SAM_SMA\201711\SAM\supporting_files\SAM_spec_lib_2017.sli'
  ; directory with mask images
  mskdirectory = 'X:\delta_sav\raster\masks\201711\fl_edge\'
  ; bad bands file path and name
  badbandfilename = 'X:\delta_sav\raster\classification\SAM_SMA\201711\SAM\supporting_files\SAM_badbands_2017.csv'
  ...
  outsuf = '_sam_rules.img'       ; output  rule image suffix
  clssuf = '_sam_class.img'       ; output class image suffix 
  ...
  spec_names_exp[1:num_endmemb] = spec_names
  spec_names_exp[0] = 'Unclassified'
  ...
  ; setting up threshholds for the number of classes
  thresh = fltarr(num_endmemb+1)
  thresh[0] = 0
  thresh[1:num_endmemb] = 0.1

  ; lookup table for class colors
  lookup = bytarr(3,num_endmemb+1)
  
  for i=1,num_endmemb do begin
  ...
  for i = startfile, endfile do begin
    ...
    outclass = outclassdir  + STRMID(basename,0,STRPOS(basename, insuf, /reverse_search)) + clssuf

    ; check if SMA file already exists
    if (file_test(outname)  eq 1) then continue
    if (file_test(outclass) eq 1) then continue

    envi_open_file, imagename,      r_fid = fid
    envi_open_file, mskimgarray[procfile_no], r_fid = mfid
    
    envi_file_query,fid,nl=imagenl,ns=imagens
    
    dims = [-1l, 0, imagens-1, 0, imagenl-1]
    
    envi_doit,'class_doit', fid = fid, pos = goodpos, dims = dims, class_names = spec_names_exp, lookup = lookup, $
              rule_fid = samrfid, rule_out_name = outname, mean = slidbgood, out_name = outclass, method = 3, $
              rule_out_bname = spec_names_exp, r_fid = samcfid, in_memory = 0, thresh = thresh, m_fid = mfid, m_pos = 0

;    envi_doit,'unmix_doit', fid = fid, pos = goodpos, dims = dims, endmem = slidbgood, out_name = outname, $
;               out_bname = spec_names_exp, r_fid = lsufid, in_memory = 0, m_fid = mfid, m_pos = 0
    print, 'completed processing file: ', basename
    
    envi_file_mng, id=samrfid, /remove
    envi_file_mng, id=samcfid, /remove
